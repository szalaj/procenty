...
class Kredyt:
    ...
    def __post_init__(self):
        ...
        if self.rodzajRat == "malejace_met2":
            ...
            odsetki_wibor = [
                float(x["odsetki_wibor"]) for x in self.kredyt_wynik["raty"]
            ]
            ...
            for i in range(len(self.kredyt_wynik["raty"])):
                self.kredyt_wynik["raty"][i]["odsetki"] = str(
                    Decimal(nowe_odsetki[i]).quantize(grosze, ROUND_HALF_UP)
                )
                self.kredyt_wynik["raty"][i]["odsetki_marza"] = nowe_odsetki_marza[i]
                self.kredyt_wynik["raty"][i]["odsetki_wibor"] = nowe_odsetki_wibor[i]

                nowa_rata = Decimal(
                    nowe_odsetki[i] + float(self.kredyt_wynik["raty"][i]["kapital"])
                )
                self.kredyt_wynik["raty"][i]["rata"] = str(
                    nowa_rata.quantize(grosze, ROUND_HALF_UP)
                )
    ...
    def _dodaj_koszt(self, dzien_koszt: dt.datetime, kwota: Decimal):
        ...
        self.wszystkie_koszty.append(
            {
                "dzien": dzien_koszt.strftime("%Y-%m-%d"),
                "kwota": float(copy.copy(self.wszystkie_koszty_kwota)),
            }
        )
    ...
    def pokaz_koszty_real(self, inflator: Inflacja):
        ...
        for koszt in self.wszystkie_koszty:
            wynik.append(
                {
                    "dzien": koszt["dzien"],
                    "kwota": inflator.urealnij(
                        dt.datetime.strptime(koszt["dzien"], "%Y-%m-%d"), koszt["kwota"]
                    ),
                }
            )
        return wynik
    ...
    def zrob_nadplate(self, dzien_nadplaty: dt.datetime, kwota: Decimal):
        ...
        grosze = decimal.Decimal(".01")
        self.wynik_nadplaty.append(
            {
                "dzien": str(dzien_nadplaty.strftime("%Y-%m-%d")),
    ...
    def _symuluj(self):
        ...
        return {
            "raty": self.wynik,
            "nadplaty": self.wynik_nadplaty,
            "kapital_na_koniec": str(self.K.quantize(decimal.Decimal("0.01"))),
        }
    ...
    def xirr(self) -> float:
        ...
        cashflows = cashflows + [
            (dt.datetime.strptime(x["dzien"], "%Y-%m-%d"), float(x["kwota"]))
            for x in self.kredyt_wynik["nadplaty"]
        ]
    ...
    def podsumowanie(self) -> str:
        ...
        dane = {
            "raty": self.kredyt_wynik["raty"],
            "info": {
                "suma_rat": str(Decimal(suma).quantize(grosze, ROUND_HALF_UP)),
                "suma_nadplat": str(
                    Decimal(suma_nadplat).quantize(grosze, ROUND_HALF_UP)
                ),
                "suma_odsetek": str(
                    Decimal(suma_odsetek).quantize(grosze, ROUND_HALF_UP)
                ),
                "liczba_rat": ile,
                "xirr": str(self.xirr),
