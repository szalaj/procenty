...
class Kredyt:
    def __init__(
        self, K: Decimal, N: int, p: Decimal, start: dt.datetime, rodzajRat: str
    ):
...
def create_kredyt(dane_kredytu, rodzajRat) -> Kredyt:
    ...
    for dzien_splaty in dane["daty_splaty"]:
        kr.zdarzenia.append(
            Zdarzenie(
                dt.datetime.strptime(dzien_splaty, "%Y-%m-%d"),
                Rodzaj.SPLATA,
                0)
        )
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        filename="logs/loginfo.log", encoding="utf-8", level=logging.DEBUG
    )
