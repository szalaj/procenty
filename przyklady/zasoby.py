...
def merge_nodes(G, node1, node2, merged_node_id=None):
    ...
    for attr in G_new.nodes[node1]:
        if attr in G_new.nodes[node2]:
            ...
            if isinstance(G_new.nodes[node1][attr], dict) and isinstance(
                G_new.nodes[node2][attr], dict
            ):
                G_new.nodes[merged_node_id][attr] = {}
                for key in set(G_new.nodes[node1][attr].keys()) | set(
                    G_new.nodes[node2][attr].keys()
                ):
                    G_new.nodes[merged_node_id][attr][key] = G_new.nodes[node1][
                        attr
                    ].get(key, 0) + G_new.nodes[node2][attr].get(key, 0)
            # For numeric values like 'produkcja', sum them
            elif isinstance(G_new.nodes[node1][attr], (int, float)) and isinstance(
                G_new.nodes[node2][attr], (int, float)
            ):
                G_new.nodes[merged_node_id][attr] = (
                    G_new.nodes[node1][attr] + G_new.nodes[node2][attr]
                )
